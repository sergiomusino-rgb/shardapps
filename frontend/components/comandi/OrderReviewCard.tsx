...
export default function OrderReviewCard({ extraction, onOrderConfirmed, className = '' }: OrderReviewCardProps) {
  ...
  const handleConfirm = async () => {
    if (items.length === 0) {
      setError('Nessuna riga da salvare: aggiungi almeno un prodotto prima di confermare.');
      return;
    }
    ...
    try {
      ...
      if (!result.success || !result.orderId) {
        setError(result.error || 'Errore nel salvataggio dell\'ordine');
        return;
      }
      ...
      setError('Errore di comunicazione con il server durante il salvataggio');
    } finally {
      setIsSaving(false);
    }
  };
  ...
  if (savedOrderId) {
    return (
      <div className={`bg-gray-800 border border-green-700/50 rounded-xl p-6 flex flex-col items-center gap-3 text-center ${className}`}>
        ...
        <p className="text-white font-semibold">Ordine salvato correttamente</p>
        <p className="text-sm text-gray-400">ID ordine: {savedOrderId}</p>
      </div>
    );
  }
    ...
    <div className={`bg-gray-800 border border-gray-700 rounded-xl p-6 flex flex-col gap-5 ${className}`}>
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="flex-1 min-w-[200px]">
          <p className="text-xs font-semibold uppercase tracking-widest text-gray-500 mb-1">Riepilogo ordine vocale</p>
        ...
        >
          ...
          {isHighConfidence ? `Confidenza alta (${confidencePct}%)` : `Richiede conferma manuale (${confidencePct}%)`}
        </span>
      </div>
        ...
        <table className="w-full min-w-[560px] text-sm">
          <thead>
            <tr className="text-left text-xs uppercase tracking-wide text-gray-500 border-b border-gray-700">
              ...
              <th className="px-2 py-2 font-medium">Unità</th>
              <th className="px-2 py-2 font-medium text-right">Prezzo Unit.</th>
              <th className="px-2 py-2 font-medium text-right">Subtotale</th>
              <th className="px-2 py-2 font-medium" aria-label="Azioni" />
            </tr>
          </thead>
          <tbody>
            {items.length === 0 && (
              <tr>
                <td colSpan={7} className="px-2 py-6 text-center text-gray-500">
                  Nessuna riga d'ordine. Tutte le righe sono state rimosse.
                </td>
              </tr>
            )}
            {items.map((item) => {
                ...
                <tr key={item._key} className="border-b border-gray-700/60 last:border-b-0">
                  ...
                  <td className="px-2 py-2.5">
                    <div className="flex items-center gap-1.5">
                      <button
                        ...
                        aria-label={`Diminuisci quantità di ${item.matched_name}`}
                      >
                  ...
                  <td className="px-2 py-2.5 text-right">
                    <button
                      ...
                      aria-label={`Rimuovi ${item.matched_name} dall'ordine`}
                    >
      ...
      {extraction.discarded_items.length > 0 && (
        <div className="rounded-lg border border-amber-700/40 bg-amber-900/10 p-3">
          <p className="flex items-center gap-1.5 text-xs font-semibold uppercase tracking-wide text-amber-400 mb-2">
            ...
            Articoli scartati o non riconosciuti ({extraction.discarded_items.length})
          </p>
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 pt-2 border-t border-gray-700">
        <div>
          <p className="text-xs uppercase tracking-wide text-gray-500">Totale Ordine</p>
        ...
        >
          ...
          {isSaving ? 'Salvataggio…' : 'Conferma e Salva Ordine'}
        </button>
      </div>
